...
function new_daf_api(){
	...
	while( $v = sqlsrv_fetch_array( $pedidos) ) {
		...
		'', '$cliente', 'API-DAFITI CARGADO DESDE APP ECOMMERCE', '', '', 1, 20, ' 0000000000000', '', '$numPedido', 0, 'E', 0, 'A', 'A', 'A', 0, 101,
		$totalPedi, 'FD', '', (SELECT RIGHT('0'+CAST(DATEPART(HOUR, GETDATE()) AS VARCHAR), 2)+RIGHT('0'+CAST(DATEPART(MINUTE, GETDATE()) AS VARCHAR), 2)+RIGHT('0'+CAST(DATEPART(SECOND, GETDATE()) AS VARCHAR), 2)), '', 3, 0, '', '1800-01-01', '', '', '', '1800-01-01',
		'', '', '', '', 0, '1800-01-01', '', NULL, 
		0, '', 0, '', '', '', '', 
		0, '', 0, '', ''
		)
